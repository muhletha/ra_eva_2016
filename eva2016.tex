...
\begin{enumerate}
    \item \textit{\textbf{Save the Peaches} (\url{http://www.savethepeaches.com/}), a 23-node network in Western Argentina which monitors temperature and humidity to be predict frost events in peach orchards.
    \item \textit{\textbf{SnowHow} (\url{http://www.snowhow.io/}), a set of 18 low-power wireless networks (945 sensors total) deployed throughout the Californian Sierra Nevada to monitor the snowpack.
    \item (current work) A Smart Building deployment in the Inria-Paris research center.
\end{enumerate}
