...
\begin{composition}
    ...
    \pers{Remy}{Leone}{Technique}[Inria, Postdoctoral Research Engineer, started Feb 2016]
    ...
    \pers{Mohamed}{Elhadad or Hadded}{PhD}[ATER Paris V, PhD Student]
...
%    \pers{Mario}{Gerla}{Visiteur}[    Professor,   UCLA,                       USA,    visit 10-20 December 2016]
    ...
    \pers{Mario}{Gerla}{Visiteur}[    Professor,   UCLA,                       USA,    visit 31 August - 23 September 10-20 December 2016]
    \pers{Travis}{Massey}{Visiteur}[  PhD Student, UC Berkeley,                USA,    visit 22 July 2016]
...
\begin{enumerate}
    \item Prof. Steven Glaser (UC Berkeley) and \thomas~recipients of the France-Berkeley Fund award for the project ``SHRIMP: Smart Harbor Implementation'', August 2016.
...
This section highlights Inria-EVA activity and contribution to the IoT-lab testbed in 2016.
...
This work done in collaboration with Anis Laouiti has been presented at the CCNC 2016 conference~\cite{khoufi:hal-01244802}.
...
This study has been presented at the MASS 2016 conference~\cite{khoufi:hal-01410069}.
...
was presented at the PEMWN 2016 conference~\cite{khoufi:hal-01410071}.
...
This work was done in cooperation Mohamed-Amine Koulali and Abdellatif Kobbane~\cite{khoufi:hal-01410082}. 
...
Some results have been presented as a poster at the IPCCC 2016 conference~\cite{khoufi:hal-01410080}.
...
This study has been presented at the AdHocNow 2016 conference~\cite{livolant:hal-01302966}.
...
This work has been done in collaboration with Mohammed Tahar Hammi from Telecom ParisTech and presented at the PEMWN 2016 conference~\cite{hammi:hal-01417671}.
...
This study has been presented at the IWCMC 2016 conference~\cite{benhassine:hal-01411119}.

In the paper presented at the WiMob 2016 conference~\cite{benhassine:hal-01411119}, we apply these prediction methods to caching.
We first selected the best experts in charge of predicting the popularity of video contents in real traces of YouTube.
We tuned the parameters of the DES expert.
We proved that the well-known LFU caching strategy can also be considered as a prediction based strategy on the Basic expert.
Simulation results show that the DES prediction-based caching strategy provides similar Hit Ratio to the well-known LFU caching strategy.
These results are usually close to the optimal ones that can be achieved only when knowing in advance the popularity of each video content for the next day, which is an unrealistic assumption.
The exception occurs when a content whose popularity was very poor becomes suddenly very popular with millions of solicitations.
In such a case, the accuracy of prediction methods becomes poor.
This opens a research direction where the knowledge of societal events is taken into account to improve the prediction.
...
\item \textbf{Nadjib Achir}
        \begin{enumerate}
            \item Sensor Networks (MDPI)
            \item  Wireless Communications and Mobile Computing (Wiley)
            \item  Internet of Things Journal (IEEE)
            \item  Ad Hoc Networks Journal (Elsevier)
        \end{enumerate}
     \item \textbf{Selma Boumerdassi}
        \begin{enumerate}
            \item  The Journal of Human-centric Computing and Information Sciences (Springer).
        \end{enumerate}
    \item \pascale
        \begin{enumerate}
    ...
    \item \paul
        \begin{enumerate}
            \item Annals of telecommunications
            \item Ad Hoc Networks,
            \item Journal of Distributed Sensor Networks,
            \item Journal of Networked and Distributed Computing,
            ...
            \item International Journal of Distributed Sensor Networks. Hindawi. 
        \end{enumerate} 
\end{itemize}
...
\begin{itemize}
    ...
    \item \thomas~and Tengfei Chang consulted for ETSI as part of the ``panel of experts'' to prepare the Test Description and Golden Image for the ETSI 6TiSCH 3 plugtests, held on 15-16 July 2016 in Berlin, Germany.
    \item \thomas~and Tengfei Chang consulted for ETSI as part of the ``panel of experts'' to prepare the Test Description and Golden Image for the ETSI 6TiSCH 2 plugtests, held on 2-4 February 2016 in Paris, France.
\end{itemize}
...
\begin{itemize}
    ...
    \item PhD in progress:
        \begin{itemize}
            \item Nesrine Ben  Hassine ``Learning algorithms for network resource management'', UVSQ, Sept 2017 (expected), \pascale~adviser, co-adviser Dana Marinca. 
             \item Younes Bouchaala   ``Handling Safety Messages in Vehicular Ad Hoc Networks (VANETs)'', UVSQ, Sept 2017 (expected), \paul~adviser. 
            \item Keoma Brun-Laguna ``Limits of Time Synchronized Channel Hopping'' \textit{(tentative title)}, EDITE - UPMC, Dec 2018 (expected), \thomas~adviser.
            \item Jonathan Mu\~noz ``km-scale Industrial Networks'' \textit{(tentative title)}, EDITE - UPMC, Oct 2018 (expected), \thomas~adviser.
        \end{itemize}
\end{itemize}
...
Pieces of the same demo have been presented to all people and teams visiting the lab.
For example, the high school students from the Lycee Louis-Le-Grand on 1 June 2016.
...
\begin{enumerate}
    ...
    \item Keoma Brun-Laguna presented the PEACH project during the annual Math exhibition in Paris, on 28 May 2016.
...
\begin{itemize}
    ...
    \item The Inria-EVA team maintains the Twitter accounts:
        \begin{itemize}
            \item EVA (\url{https://twitter.com/InriaEVA}), 180 tweets
            \item OpenWSN (\url{https://twitter.com/openWSN}), 88 tweets
            \item savethepeaches (\url{https://twitter.com/peachesthesave}),  73 tweets
            \item HeadsUp! (\url{https://twitter.com/heads_up_tech}), 28 tweets
            \item REALMS team (\url{https://twitter.com/realms_team}), 28 tweets
            \item SOLsystem (\url{https://twitter.com/S0Lsystem}), 16 tweets
            \item snowhow (\url{https://twitter.com/snowhow_io}), 14 tweets
        \end{itemize}
\end{itemize}
